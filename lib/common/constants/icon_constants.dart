class IconConstants {
  static const path = 'assets/icons';

  static const icCancer = path + '/ic_cancer.png';
  static const icPisces = path + '/ic_pisces.png';
  static const icMale = path + '/ic_male.png';
  static const icFemale = path + '/ic_female.png';
  static const icCalendar = path + '/ic_calendar.svg';
  static const icHome = path + '/ic_home.svg';
  static const icMenstrualCycle = path + '/ic_menstrual_cycle.svg';
  static const icAdd = path + '/ic_add.svg';
  static const icClose = path + '/ic_close.svg';
  static const icHeart = path + '/ic_heart.svg';
}