...
class TopSnackBarRoute<T> extends OverlayRoute<T> {
  ...
  final SnackBarWidget topSnackBar;
  ...
  TopSnackBarRoute(
      {required this.theme, required this.topSnackBar, RouteSettings? settings})
    ...
    if (_onStatusChanged != null) {
      switch (status) {
        case AnimationStatus.completed:
          ...
          _onStatusChanged!(currentStatus ?? TopSnackBarStatus.showing);
        ...
        case AnimationStatus.forward:
          ...
          _onStatusChanged!(currentStatus ?? TopSnackBarStatus.isAppearing);
          break;
        case AnimationStatus.reverse:
          currentStatus = TopSnackBarStatus.isHiding;
          _onStatusChanged!(currentStatus ?? TopSnackBarStatus.isHiding);
          if (overlayEntries.isNotEmpty) {
        ...
        case AnimationStatus.dismissed:
          ...
          _onStatusChanged!(currentStatus ?? TopSnackBarStatus.dismissed);
          if (!isCurrent) {
  ...
  AnimationController createAnimationController() {
    ...
    return AnimationController(
      ...
      vsync: navigator ?? NavigatorState(),
    );
  }
    ...
    return AlignmentTween(begin: _initialAlignment, end: _endAlignment).animate(
      CurvedAnimation(
        parent: _animationController ??
            AnimationController(vsync: NavigatorState()),
  ...
  TickerFuture didPush() {
    ...
    _configureTimer();
    super.didPush();
    return _animationController?.forward() ?? TickerFuture.complete();
  }
  ...
  Iterable<OverlayEntry> createOverlayEntries() {
    ...
    final alignTransition = AlignTransition(
        alignment: _animation ?? createAnimation(),
...
TopSnackBarRoute showSnackBar<T>(
    {required BuildContext context, required SnackBarWidget topSnackBar}) {
